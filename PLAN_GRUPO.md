...
- [x] **Notificaciones locales** (2026-06-11): campana del Home → sheet de config con chips de avisos ("15 min antes", agregar 5/10/30/60; vacío = apagadas). Config en `users/{uid}.notificationLeadTimes` (sincroniza entre dispositivos). `NotificationService` (`lib/services/notification_service.dart`): se suscribe a cursos/recordatorios/periodos/config, debounce de 2s y reprograma TODO reutilizando `buildToday`/`buildUpcomingDays(7)` — los avisos siempre coinciden con lo que muestra el Home. Modo `inexactAllowWhileIdle` (sin permiso de alarmas exactas). Permiso `POST_NOTIFICATIONS` pedido al guardar. Manifest: permiso + receivers de reinicio. Deps nuevas: `flutter_local_notifications`, `flutter_timezone`, `timezone` (correr `flutter pub get`). Recordatorios SIN hora no generan avisos. Al cerrar sesión se cancelan todos.
  - **Aviso de compartidos recibidos:** cuando llega algo nuevo a la bandeja, notificación instantánea "X te compartió un curso: ...". LIMITACIÓN (documentar en la susten): funciona con la app abierta o en segundo plano — push real con la app cerrada requeriría FCM + Cloud Functions (plan de pago de Firebase), fuera del alcance. Los avisos de "tu curso empieza en N min" SÍ llegan siempre (los agenda el sistema Android).

- [x] **Recordatorios de voz** (2026-06-11, diseño cambiado por el equipo: GRABACIÓN en vez de transcripción): pestañas **Escritos | Grabados** en Recordatorios. Un grabado es un Reminder normal (misma frecuencia/fecha/hora → sale en Home y LE LLEGAN NOTIFICACIONES) donde el audio reemplaza a las notas escritas. Grabación máx. 2 min (`record`, AAC), reproducción en la card (`audioplayers`, un player compartido: reproducir uno detiene el anterior). El archivo vive LOCAL en `<docs>/reminder_audios/` (`audioFileName` en el modelo) — LIMITACIÓN documentada: el audio NO se sincroniza entre dispositivos del mismo usuario. **Compartir audios SÍ funciona** (2026-06-11): el audio viaja como Base64 dentro del payload de `shared_items` (por eso la grabación es calidad voz: 32 kbps mono → 2 min ≈ 480 KB, bajo el límite de 1 MB por doc de Firestore); al aceptar, el receptor lo materializa como archivo local. Audios viejos muy pesados (>700 KB, grabados antes del ajuste de bitrate) no se pueden compartir y avisan. En la bandeja, los recordatorios con audio muestran ícono de micrófono. Limpieza de archivos: al eliminar el recordatorio, al re-grabar y al salir sin guardar. Permiso `RECORD_AUDIO` en runtime. Storage en la nube descartado (Firebase Storage requiere plan de pago).
  - **Nota de voz rápida** (estilo WhatsApp): en la pestaña Grabados, MANTENER PRESIONADO el botón + graba mientras se sostiene y al soltar guarda al instante como "Nota de voz dd/mm hh:mm" SIN programar (frecuencia "Una vez" sin fecha → no aparece en Home ni notifica hasta que se edite y se le ponga fecha — el formulario la exige al guardar). Toque corto del + = formulario completo. Soltar antes de 1s = descarte (toque accidental).

**ROADMAP COMPLETO — no quedan features pendientes.** 🎉
